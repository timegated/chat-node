...
export default {
  ...
  methods: {
    async createFineTune (): Promise<void> {
      try {
        ...
        const post = await axios.post(`${BASE_URL_DEV}/fine-tune`, { id });
        console.log(post);
      } catch (error) {
        console.error(error);
        throw error;
    ...
    async retrieveStatus(): Promise<void> {
      try {
        const id = this.fileId;
        const getStatus = await axios.get(`${BASE_URL_DEV}/fine-tune/status/${id}`);
  ...
  async created() {
    const available = await axios.get(`${BASE_URL_DEV}/files/list`);
